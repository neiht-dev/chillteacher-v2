...
const Schools: React.FC = () => {
	...
	const handleModalOk = () => {
		form.validateFields().then((values) => {
			if (selectedSchool) {
				setSchools(schools.map((s) => (s.id === selectedSchool.id ? { ...s, ...values } : s)));
	...
	const columns: ColumnsType<School> = [
		...
		},
		{
			...
			dataIndex: 'type',
			key: 'type',
			render: (text) => <Tag color={text === 'Public' ? 'blue' : 'gold'}>{text}</Tag>,
		},
		{
			title: 'Status',
			dataIndex: 'status',
		...
		},
		{
			...
			render: (_, record) => (
				<Space>
					<Button type="text" icon={<EyeOutlined />} onClick={() => handleViewSchool(record)} />
					<Button type="text" icon={<EditOutlined />} onClick={() => handleEditSchool(record)} />
					<Button
						type="text"
						danger
						icon={<DeleteOutlined />}
						...
						<List
							...
							dataSource={[
								...
								},
								{
									label: 'Status',
									value: (
										<Tag color={getStatusColor(selectedSchool.status)}>{selectedSchool.status}</Tag>
									),
								},
							]}
							renderItem={(item) => (
