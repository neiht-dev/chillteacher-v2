...
const Classroom: React.FC = () => {
			...
			students.map((student) =>
				student.id === studentId
					? {
							...student,
							status,
							arrivalTime: status === 'present' ? dayjs().format('HH:mm') : undefined,
						}
					: student
			)
		);
		message.success(`Attendance updated for ${students.find((s) => s.id === studentId)?.name}`);
	};
	...
	const handleBehaviorChange = (
		...
		setStudents(
			students.map((student) => (student.id === studentId ? { ...student, behavior } : student))
		);
		message.success(`Behavior updated for ${students.find((s) => s.id === studentId)?.name}`);
	};
				...
				{selectedClassData && (
					...
					>
						<Row gutter={16}>
							...
							<Col>
								<Text strong>Grade:</Text> {selectedClassData.grade}-{selectedClassData.section}
							</Col>
							<Col>
