...
run_drizzle_migrations() {
    echo "Running Drizzle migrations inside Docker container..."
    # Replace 'nextjs-app' with your actual service/container name if different
    docker compose exec nextjs-app npm run db:migrate

    if [ $? -eq 0 ]; then
        echo "Drizzle migrations completed successfully!"
    else
        echo "Error: Drizzle migrations failed!"
        return 1
    fi
}

# Main logic for setup-server.sh
case "$1" in
    "rebuild"|"no-rebuild")
        setup_application "$1"
        ;;
    "run-migrations")
        ...
        ;;
    *)
        echo "Usage: $0 {rebuild|no-rebuild|run-migrations}"
        exit 1
        ;;
esac
